  ...
  <div class="contain">
    ...
    >
      <el-form-item prop="discuss_name" class="ser_option1">
        <el-input
          class="input"
          style="background-color:#00002D;border:1px solid #1B4E79;padding-left:0px;padding-right:0px"
          v-model="ruleForm.discuss_name"
          placeholder="评分者姓名/手机号"
        ></el-input>
      </el-form-item>
      <el-form-item prop="expert_name" class="ser_option2">
        <el-input
          style="background-color:#00002D;border:1px solid #1B4E79;"
          v-model="ruleForm.expert_name"
      ...
      >
        ...
        <el-table-column label="评分" prop="name">
          <template v-slot="scope">
            ...
            <div class="discuss_name">
              {{ scope.row.user_name }}{{ scope.row.user_mobile }}
            </div>
          </template>
        </el-table-column>
        <el-table-column
          label="评分时间"
          class-name="pingfen_time"
          prop="addtime"
        >
        </el-table-column>
        ...
        <el-table-column label="操作">
          <template v-slot="scope">
            ...
            >
              {{ scope.row.isshow == 0 ? "显示评分" : "隐藏评分" }}</el-button
            >
            <el-button
              size="mini"
              type="text"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        question: [{ message: "请输入问题", trigger: "blur" }],
  ...
  created() {
    ...
    if (h > 1080 || h == 1080) {
      var shengyu = h - 231 - 146 - 60;
    } else {
      var shengyu = h - 168 - 146 - 60;
    }
  ...
  methods: {
    ...
    handleCurrentChange: function(currentPage) {
      ...
      if (
        (this.ruleForm.expert_name ||
          this.ruleForm.discuss_name ||
          this.ruleForm.discuss_name ||
    ...
    clickSure() {
      ...
      if (this.isshow == 1) {
        this.$axios
          .fetchPost("Admin/Api/update_appraises", {
            aId: this.aid,
            opt: "hidden"
          })
          .then(res => {
