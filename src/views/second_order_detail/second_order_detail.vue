<template>
  <div class="jianjie">
    ...
    <div class="left">
      ...
      <div class="allprice">
        总金额：¥<span class="price">{{ this.coastInt }}</span
        >.{{ this.coastFloat == null ? "00" : this.coastFloat }}
      </div>
    ...
    <ul class="right">
      <li
        v-for="(item, index) in this.order_info.products"
        :key="index"
        @click="watchGooddetail(item)"
      >
        ...
        <div class="text">
          ...
          <div class="guige">商品规格：{{ item.spec }}</div>
        </div>
        <div class="buy_number">
          <span class="fuhao">¥</span>{{ item.price
          }}<span class="fuhao">*</span>{{ item.quantity }}
        </div>
      </li>
    </ul>
  </div>
...
export default {
  ...
  methods: {
    ...
    getOrederinfo(Id) {
      this.$axios
        .fetchPost("/Home/Order/GetMpOrderDetail", { Id: Id })
        .then(res => {
    ...
    watchGooddetail(item) {
      //跳转到 商品详情页面
      this.$router.push({ path: "/goods_detail", query: { id: item.spec_sn } });
    }
  }
};
</script>
