<template>
  <div class="contain">
    <div class="sel_city" ref="sel_city">
      <div
        :class="[index == 0 ? 'kuang cities cities1' : 'kuang cities']"
        ref="cur_city"
        v-for="(item, index) in areadata"
        :key="index"
        @click="sel_city(item, index)"
      >
        {{ item.name }}
      </div>
    </div>
    ...
    <div class="hospital_num">共{{ this.total }}个结果</div>
...
export default {
    ...
    this.$axios
      ...
      .then(res => {
        if (res.data.code == "200" && res.data.message == "请求成功") {
          ...
          if (h > 1080 || h == 1080) {
            this.$refs.hospitalinfo.style.height = `${this.getheight(
              204,
              0.085,
              0.025
            )}px`;
  ...
  methods: {
    ...
    sel_city(item, index) {
      ...
      if (
        window.sessionStorage.getItem("isstore") == "1" ||
        window.sessionStorage.getItem("isstore") == 1
      ) {
