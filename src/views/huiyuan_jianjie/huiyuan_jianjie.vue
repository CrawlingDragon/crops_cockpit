<template>
  <div class="jianjie">
    <div class="left">
      <el-image class="img" fit="cover" :src="this.huiyuaninfo.avatar">
    ...
    <div class="right">
      <div class="name">
        ...
        <span class="id">ID:{{ this.huiyuaninfo.id }}</span>
      </div>
      <div class="right_left">
        <span class="text1">手机：{{ this.huiyuaninfo.mobile }}</span>
        <span class="text1">村名：{{ this.huiyuaninfo.address }}</span>
        <span class="text1">加入时间：{{ this.huiyuaninfo.inputtime }}</span>
        <span class="text1"
          >种养种类：
          <p class="zuowu">
...
export default {
  ...
  methods: {
    getpersonInfo(Id, appId) {
      this.$axios
        .fetchPost("/Home/Member/GetMpUserDetail", { Id: Id, appId: appId })
        .then(res => {
